<script lang="ts">
	...
	let colorName = $derived.by(() => { return colorHex ? hexToColorName(colorHex) : "Black"});
...
>
    {#if _submitted}
        <div in:fade={{ duration: 500, delay: 100 }} class="flex flex-col justify-center items-center my-12 space-y-2">
            ...
            <button in:fade={{ duration: 600, delay: 3000 }} onclick={() => goto('/')} class="mt-6" style="padding: 6px 18px;">Back Home</button>
        </div>
	{:else}
    	...
    	>
       	...
       	>
      		<div>
     			<label for="make" class="block text-sm text-gray-500 mb-1">
        				Make
     			</label>
     			<input
        				id="make"
            ...
            disabled={_submitting}
      		...
      		<div>
     			<label for="model" class="block text-sm text-gray-500 mb-1">
        				Model
     			</label>
     			<input
        				id="model"
            ...
            <section
      		class="grid grid-cols-2 md:grid-cols-3 gap-4 items-end mt-4"
       	>
      		<!-- Color -->
      		<div class="grow">
     			<label hidden for="color" class="block text-sm text-gray-500 mb-1">
        				Color
     			</label>
                    <ColorPicker label={colorName} bind:hex={colorHex} components={ChromeVariant} sliderDirection="horizontal"
                        nullable --input-size="36px" --focus-color="amber"  />
     			<input type="hidden" id="color" name="color" value={colorHex ?? ''} />
      		</div>

      		<!-- Year -->
      		<div class="shrink">
     			<label hidden for="year" class="block text-sm text-gray-500 mb-1">
        				Year
     			</label>
     			<input
                        id="year"
      		...
      		<!-- For Sale -->
      		<div class="col-span-2 md:col-auto flex items-center gap-3 h-10 mt-6 sm:mt-0 justify-center md:justify-end">
